...
mongoose.connect('mongodb://admin:Ebony123$@ds115340.mlab.com:15340/eb')
...
io.on('connection', socket => {
    socket.on('join', prefs => {
        ...
        socket.on('bill', d => {
            socket.to(room).emit('bill', d)
        })
        socket.on('select', d => {
            socket.to(room).emit('select', d)
        })
        socket.on('deselect', d => {
            socket.to(room).emit('deselect', d)
        })
        socket.on('finish', receipt => {
            socket.to(room).emit('finish', receipt)
